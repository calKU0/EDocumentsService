    ...
    public class Worker : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    using var scope = _scopeFactory.CreateScope();
                    var printingService = scope.ServiceProvider.GetRequiredService<EReturnsService>();
                    var fileService = scope.ServiceProvider.GetRequiredService<IFileService>();

                    await printingService.GenerateAndSendEReturns(stoppingToken);

                    string returnsPath = Path.Combine(AppContext.BaseDirectory, ServiceConstants.ExportFolder);
                    fileService.DeleteFilesFromFolder(returnsPath, ".pdf");
                    _logger.LogInformation("Files deleted from {Path}", returnsPath);
