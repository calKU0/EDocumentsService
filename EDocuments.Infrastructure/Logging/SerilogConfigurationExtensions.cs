...
namespace EDocuments.Infrastructure.Logging
{
    public static class SerilogConfigurationExtensions
    {
        private const string DefaultOutputTemplate = "[{Timestamp:yyyy-MM-dd HH:mm:ss} {Level:u3}] {Message:lj}{NewLine}{Exception}";
        public static LoggerConfiguration ConfigureServiceLogging(this LoggerConfiguration loggerConfiguration, IConfiguration configuration, string serviceName)
        {
            ...
            ConfigureEmailSink(loggerConfiguration, configuration, serviceName);
            ConfigureSeqSink(loggerConfiguration, configuration);

            return loggerConfiguration;
        }

        private static void ConfigureEmailSink(LoggerConfiguration loggerConfiguration, IConfiguration configuration, string serviceName)
        {
            ...
            var emailSinkOptions = new EmailSinkOptions
            {
                ...
                Subject = new MessageTemplateTextFormatter(configuration["SerilogEmail:Subject"] ?? $"{serviceName} - Errors", null),
